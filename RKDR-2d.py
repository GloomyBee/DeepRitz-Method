...
class Utils:
    @staticmethod
    def gaussian_kernel(r, s, device):
        """高斯核函数"""
...
# 模型模块
class EnhancedRitzNet(nn.Module):
    ...
    def compute_kernel_features(self, x):
        """计算高斯核特征"""
        dx = x[:, 0:1] - self.nodes[:, 0:1].T  # (batch_size, num_nodes)
        dy = x[:, 1:2] - self.nodes[:, 1:2].T  # (batch_size, num_nodes)
        r = torch.sqrt(dx**2 + dy**2)  # (batch_size, num_nodes)
...
class Trainer:
    ...
    def test(self):
        ...
        target = Utils.exact(data).to(self.device)
    ...
    def train(self):
        ...
        with open(os.path.join(self.data_dir, "rkdr_loss_history.txt"), "w") as f:
            ...
            for step in range(self.params["trainStep"]):
                ...
                target_boundary = Utils.exact(data_boundary).to(self.device)
...
class Visualizer:
    ...
    def plot_result(self, model, device, train_time, test_time):
        ...
        exact_sol = Utils.exact(coords_tensor).detach().cpu().numpy().reshape(n_sample, n_sample)

        with open(os.path.join(self.data_dir, "rkdr_solution_data.txt"), "w") as f:
    ...
    def plot_h1_error_distribution(self, model, device, h1_error):
        ...
        exact_sol = Utils.exact(coords_tensor)
        ...
        grad_pred_x = grad_pred[:, 0].reshape(n_sample, n_sample).detach().cpu().numpy()
        grad_pred_y = grad_pred[:, 1].reshape(n_sample, n_sample).detach().cpu().numpy()
        grad_exact_x = grad_exact[:, 0].reshape(n_sample, n_sample).detach().cpu().numpy()
        grad_exact_y = grad_exact[:, 1].reshape(n_sample, n_sample).detach().cpu().numpy()

        pred = pred.reshape(n_sample, n_sample).detach().cpu().numpy()
        exact_sol = exact_sol.reshape(n_sample, n_sample).detach().cpu().numpy()

        abs_error = np.abs(pred - exact_sol)
        grad_error = np.sqrt((grad_pred_x - grad_exact_x)**2 + (grad_pred_y - grad_exact_y)**2)

        l2_error = np.sqrt(np.mean(abs_error ** 2))
        grad_l2_error = np.sqrt(np.mean(grad_error ** 2))
...
def main():
    ...
    params["dd"] = 1
    params["radius"] = 1.0
    params["bodyBatch"] = 4096
    params["bdryBatch"] = 4096
    params["lr"] = 0.001
    params["preLr"] = 0.01
    params["width"] = 100
    params["depth"] = 3
    ...
    torch.save(model.state_dict(), os.path.join(trainer.data_dir, "rkdr_last_model.pt"))
    with open(os.path.join(trainer.data_dir, "rkdr_training_time.txt"), "w") as f:
