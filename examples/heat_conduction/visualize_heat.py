...
def plot_results(model, pde, loss_history, save_dir: str):
    ...
    ax1.plot(vertices[:, 0], vertices[:, 1], 'k-', linewidth=1.5)
    ...
    ax3.plot(vertices[:, 0], vertices[:, 1], 'k-', linewidth=1.5)
    ...
    plt.plot(vertices[:, 0], vertices[:, 1], 'k-', linewidth=1.5)
    plt.annotate('T=20°C\n(Dirichlet)', xy=(0, 0.5), xytext=(-0.5, 0.5),
                 arrowprops=dict(arrowstyle="->", color='blue'),
                 fontsize=10, ha='center', va='center', color='blue',
                 bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='blue'))
    plt.annotate('q=-100 W/m²\n(Neumann)', xy=(0.5, 1.0), xytext=(0.5, 1.25),
                 ...
                 fontsize=10, ha='center', va='center', color='red',
                 bbox=dict(boxstyle='round', facecolor='white', alpha=0.8, edgecolor='red'))
    plt.annotate('Adiabatic (q=0)', xy=(1.0, 0.0), xytext=(1.0, -0.25),
                 arrowprops=dict(arrowstyle="->", color='black'),
                 fontsize=10, ha='center', va='center',
                 bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    plt.title("Temperature Distribution", fontsize=14, fontweight='bold', pad=20)
    ...
    plt.plot(vertices[:, 0], vertices[:, 1], 'k-', linewidth=1.5)
    plt.title("Temperature Contour Lines", fontsize=14, fontweight='bold')
    plt.xlabel("x (m)", fontsize=12)
    plt.ylabel("y (m)", fontsize=12)
    plt.gca().set_aspect('equal')
    plt.tight_layout()
    ...
    plt.title('Temperature Distribution Histogram', fontsize=14, fontweight='bold')
...
def plot_temperature_profile(model, save_dir: str):
    """绘制沿特定路径的温度剖面"""
    model.eval()
    device = next(model.parameters()).device
    ...
    plt.tight_layout()
    save_path = os.path.join(save_dir, 'heat_profiles.png')
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"温度剖面图已保存到: {save_path}")
    plt.show()
...
def main():
    ...
    # 创建结果子文件夹
    results_dir = os.path.join(output_dir, 'heat_results')
    ...
    # 绘制结果
    print("\n生成可视化...")
    plot_results(model, pde, loss_history, results_dir)

    # 绘制温度剖面
    print("\n生成温度剖面图...")
    plot_temperature_profile(model, results_dir)
